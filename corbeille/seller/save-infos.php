...
if (!empty($lname) && !empty($fname) && !empty($title) && !empty($cin) && !empty($income)) {
    ...
    if ($result_select_temp->num_rows > 0) {
        ...
        $insert_query = "INSERT INTO `slf_user_client` (`client_id`, `email`, `phone`, `lname`, `fname`, `title`, `cin`, `income`, `region_id`) 
        VALUES ('{$client_id}','{$email}','{$phone}','{$lname}','{$fname}','{$title}','{$cin}','{$income}','{$region_id}')";
