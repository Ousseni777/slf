...
$img_name_picture = $_FILES['yourPicture']['name'];
$img_type_picture = $_FILES['yourPicture']['type'];
$tmp_name_picutre = $_FILES['yourPicture']['tmp_name'];
...
$img_explode_picture = explode('.', $img_name_picture);
$img_ext_picture = end($img_explode_picture);
...
if (!empty($img_name_picture) && !empty($img_name_cin) && !empty($img_name_adress) && !empty($img_name_rib)) {
    $query = "SELECT *  FROM SLF_USER_CLIENT  WHERE client_id = '{$client_id}'";
    $result = $conn->query($query);
    

    if ($result->num_rows > 0) {
        if (in_array($img_ext_picture, $extensions) && in_array($img_ext_cin, $extensions) && in_array($img_ext_rib, $extensions) && in_array($img_ext_adress, $extensions)) {
            $types = ["image/jpeg", "image/jpg", "image/png"];
            if (in_array($img_type_picture, $types) && in_array($img_type_cin, $types) && in_array($img_type_rib, $types) && in_array($img_type_adress, $types)) {

                $time = rand(time(), 100000000);
                $new_img_name_picture = $time . '_' . $img_name_picture;
                ...
                if (move_uploaded_file($tmp_name_picutre, "images/" . $new_img_name_picture) && move_uploaded_file($tmp_name_cin, "images/" . $new_img_name_cin) && move_uploaded_file($tmp_name_rib, "images/" . $new_img_name_rib) && move_uploaded_file($tmp_name_adress, "images/" . $new_img_name_adress)) {

                    $update_query = "UPDATE `slf_user_client` 
                SET `picture_piece`='{$new_img_name_picture}',`cin_piece`='{$new_img_name_cin}',`rib_piece`='{$new_img_name_rib}',`adress_piece`='{$new_img_name_adress}' 
