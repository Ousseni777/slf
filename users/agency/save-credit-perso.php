...
$TARIFF_ID = 0; //uniquement pour le crédit personnel et renouvelable
...
if (!empty($_POST['CLIENT_CIN'])) {
    ...
    VALUES ('{$ran_id}','{$TARIFF_ID}','{$CLIENT_CIN}','{$SELLER_ID}',0,'En attente','{$AMOUNT}','{$DURATION}','{$MONTHLY}','{$PROJECT}','{$APP_FEES}','{$DOWN_PMT_PERC}','{$DOWN_PMT}','{$ADI}','{$COST_EX_ADI}','{$today}')";
